...
public final class TestDataInitializer implements InitializingBean,
  ...
  public void afterPropertiesSet() throws Exception {
    ...
    try {
      ...
      Discount goudaDiscount = new Discount(gouda, 0.1,
          "Special season's offer");
      session.save(goudaDiscount);

      Discount edamDiscount = new Discount(edam, 0.15,
          "Fresh from the cow");
      session.save(edamDiscount);

      User regularUser = new User("user", "user", "Regular User",
          false);
      session.save(regularUser);

      User adminUser = new User("admin", "admin", "Administrator",
          true);
